...
public class HazardSpawner5 : MonoBehaviour
{
    public bool levelEnd;
    bool greenLaserIsAbove, greenLaserActive, greenLaserRunning;
    int rocketCount;
    ...
    void Start()
    {
        ...
        isRunning = false;
        cannonRunning = false;
        engineEvent = GameObject.Find("EngineParent").GetComponent<EngineEvent5>();
    ...
    void FixedUpdate()
    {
        if(greenLaserIsAbove && greenLaserActive && !levelEnd)
        {
            greenLaser.transform.Translate(Vector3.down * 3.0f * Time.deltaTime);
        }
        if (!greenLaserIsAbove && greenLaserActive && !levelEnd)
        {
            greenLaser.transform.Translate(Vector3.up * 3.0f * Time.deltaTime);
        }
        ...
        if (!greenLaserRunning && !engineEvent.questionEvent) StartCoroutine(SpawnGreenLaser());
    ...
    IEnumerator SpawnGreenLaser()
    {
        greenLaserRunning = true;
        int spawnInterval = Random.Range(1, 15);
        yield return new WaitForSeconds(spawnInterval);
        if (!engineEvent.questionEvent && !levelEnd)
        {
            greenLaserActive = true;
            audio.PlayOneShot(greenLaserSound, 0.08f);
            yield return new WaitForSeconds(5);
            greenLaserActive = false;
            if (greenLaserIsAbove == false)
            {
                greenLaserIsAbove = true;
            }
            else
            {
                greenLaserIsAbove = false;
